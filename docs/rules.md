...
Generated from the check registry — 36 rules. Every rule accepts `'error' | 'warn' | 'off'` or `[severity, options]` in `seo.config.ts`, and can be suppressed per page with `useRanklintIgnore()`.
